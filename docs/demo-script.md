...
| 1:05–1:15 | Kill | Hit kill/unfollow → badge: "This agent can no longer move your funds" → a mirror attempt against the killed follow is rejected on-chain (`PolicyInactive`), and the UI re-reads the chain to prove the follow is dead. | `KillButton` |
