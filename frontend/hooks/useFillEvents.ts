...
export function useFillEvents(agentId?: number): {
  ...
  const page = latestFillPage(count.data ?? BigInt(0), limit);
  ...
  const totalCount =
    live && typeof count.data === "bigint" ? Number(count.data) : undefined;
  ...
  return {
    fills,
    isLoading: count.isLoading || backfill.isLoading,
    refetch,
    totalCount,
    hasMore,
    loadMore,
